// Алгоритм Ф.Розенблатта обучения персептрона,
// имеющего пороговую активационную функцию.
...
// Персептрон принимает на вход вектор входных сигналов
...
int neuron(const vector<double> &u, const vector<double> &w)
{
	...
	return (x >= 0 ? 1 : -1); // пороговая функция
}
...
vector<double> rozenbl(const vector<pair<int,vector<double> > > &u, int n, double alpha)
{
	...
	{
		w.push_back((double)rand()/(double)RAND_MAX - 0.5); // заполнение весов малыми случайнми значениями
	}
